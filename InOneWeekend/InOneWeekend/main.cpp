	...
	*/
	...
	else
	{
		/*
			...
			이 2개의 교차점에 해당하는 2개의 비율값 t 들 중에서,
			더 작은 t값만 사용하겠다는 뜻. 

			즉, 이차방정식의 해 중에서 더 작은 해를 사용하겠다는 뜻!

			이것은 기하학적으로도 타당한 결정인데, 왜나햐면, 비율값 t 가 더 작은 경우,
			해당 비율값 t 는 반직선 출발점에 더 가까운 t 에 해당하고,
			우리는 main() 함수에서 모든 반직선의 출발점을 카메라 위치(0, 0, 0)으로 설정했으므로,
			더 작은 비율값 t 는 반직선 상에서 카메라에 더 가까운 지점의 비율값이라는 의미가 됨!

			따라서, 더 작은 비율값 t를 반직선의 .at(t) 로 전달하여 해당 지점의 좌표값을 계산하면,
			...
			그런데, 왜 하필 근의 공식의 ± sqrt(discriminant) 부분에서
			더 작은 해를 구하기 위해 - sqrt(discriminant) 를 사용한걸까?

			이거는 아주 당연하다.
			half_b 는 무조건 0보다 작은 음수가 나올 수밖에 없기 때문!

			half_b 는 <반직선 출발점 ~ 구체의 중점까지의 벡터> 와 <반직선 방향벡터> 의 내적으로 계산했었지?
			그런데, <반직선 출발점 ~ 구체의 중점까지의 벡터> 는 vec3(0, 0, 1) 로 계산될 수 있고,
			<반직선 방향벡터> 는 임의의 vec3(x, y, z) 중 하나겠지? 
			
			이 때, 반직선 방향벡터는 무조건 카메라 위치인 (0, 0, 0) 에서 
			음의 z축 방향쪽으로 향하도록 계산되어 있어, <반직선 방향벡터>의 z값이 무조건 음수일 수밖에 없음!

			따라서, 두 벡터를 내적하면, 각 컴포넌트끼리의 곱의 합으로 계산하게 될텐데,
			<반직선 출발점 ~ 구체의 중점까지의 벡터> 의 x, y 컴포넌트가 0이니, 
			x, y 간 컴포넌트의 곱은 0으로 나오게 되어 의미가 없고,
			z 간 컴포넌트의 곱은 1 * z = z 이고, z가 무조건 음수일 수밖에 없다고 했으므로,
			내적의 결과값 half_b 는 무조건 음수일 수밖에 없다!

			따라서, (-half_b ± sqrt(discriminant)) / a 라는 근의 공식에서,
			-half_b 도 양수, a 도 양수, sqrt(discriminant) 도 양수라면,
			상식적으로 - sqrt(discriminant) 로 계산해야 더 작은 해(= 더 작은 비율값 t)가 나오겠지?

			그래서 (-half_b - sqrt(discriminant)) / a 로 계산된 것임!
		*/
...
/*
	'so we won't worry about negative values of t yet.'


	본문에서 반직선의 negative t (음수인 비율값 t) 를 신경쓰지 않아도 된다고 한 말은,
	말 그대로 지금과 같이 구체를 렌더링하는 케이스에서는 신경쓰지 않아도 된다는 뜻임.

	즉, 카메라가 (0, 0, 0)에 존재하고,
	모든 반직선이 카메라에서 출발에서 음의 z축 쪽을 향하도록 설정되어 있고,
	구체는 (0, 0, -1) 에 위치시키고, 구체의 반지름 또한 0.5 밖에 안되므로,

	반직선 상에서 비율값 t 가 음수인 지점에서
	반직선과 구체가 교차할 일이 없기 때문!

	왜냐하면, 반직선 상의 비율값 t 가 음수라는 뜻은,
	반직선 출발점 뒷쪽에 위치하는 지점, 즉,
	'카메라 뒷쪽에 위치하는 지점'이라는 뜻인데,

	카메라 뒷쪽에 위치하려면 양의 z축 쪽을 바라보는,
	즉, z좌표값이 양수인 지점에서 교차해야 한다는 뜻임.

	그런데, 지금 우리가 렌더링하는 구체 또는 구체의 어느 부분도
	카메라 뒷쪽에 존재하지 않음.

	왜냐? 카메라에서 가장 가까운 구체의 부분조차
	구체의 반지름이 0.5 이기 때문에
	(0, 0, -0.5) 로 계산되므로,

	구체의 어느 부분들 중에서도 z좌표값이 양수인 지점이 존재하지 않음.

	즉, 정리하면,
	'카메라 뒷쪽에 위치하는 물체가 존재하지 않으므로,
	반직선 상의 음수인 비율값 t 에 대해서는 신경쓰지 않아도 된다!'
	라는 의미임!
*/